    ...
    <div>
      ...
      <div class="product_app">
        ...
        <div class="goods-item" v-for="(item,i) of list" :key="i">
          ...
          <div class="btn6">
            <button class="btn">
              <router-link :to="`/details/${item.pid}`">商品详情</router-link>
            </button>
          </div>
        </div>
      </div>
      ...
      <div class="page-bar">
        <ul>
          ...
          <!-- 当前页背景色为蓝色 -->
          <li v-for="(item,index) in indexs" :key="index" v-bind:class="{'active': cur == index+1}">
            <a class="bar" v-on:click="btnClick(index+1)">{{index+1}}</a>
          </li>
          <li v-if="cur<indexs">
